...
use PDO;
use PDOException;
use OrderApp\Core\Db\AbstractModel;
/**
...
class OrdersModel extends AbstractModel
{
    ...
    public function getOrders(array $data)
    {
        ...
        $query = sprintf("SELECT orders.*, CONCAT(firstname, ' ' , lastname) AS customer_name
                    FROM %s
                    LEFT JOIN customers ON customers.id = orders.$this->customerId
    ...
    public function save(array $data)
    {
        ...
        $query = sprintf("INSERT INTO %s ($this->date, $this->status, $this->amount, $this->desc, $this->customerId)
                          VALUES ('%s', '%s', '%d', '%s', '%d')",
            static::TABLE,
            $data['order_date'],
            $data['status_filter'],
