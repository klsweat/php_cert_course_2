...
use OrderApp\Model\ {OrdersModel, CustomersModel};
use OrderApp\Core\Controller\AbstractController;
use OrderApp\Core\Messaging\Messenger;
use OrderApp\Core\Traits\MessageTrait;
            ...
            if ($form) {
                ...
                if ($form->isValid) {
                    ...
                    if (is_array($result)) {
                        $values['data'] = $result;
                        $values['form'] = $form;
                        $values['messenger'] = $this->newMessage(OrdersModel::MSG_ORDERS_RELOADED, Messenger::TYPE_INFO);
                    }elseif(is_bool($result)){
                        //We saved, shoot back to the orders list
                        $values = $this->getDefaults();
                        $values['messenger'] = $this->newMessage(OrdersModel::MSG_ORDER_ADD, Messenger::TYPE_INFO);
                    };
                } else {
                    //Something in the form was invalid
                    $values['messenger'] = $this->newMessage('Invalid Form Entry', Messenger::TYPE_ERROR);
                }
            } else {
